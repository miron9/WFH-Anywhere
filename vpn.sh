...
function wg_vpn() {
	...
	sleep 1
	ip addr add 192.168.3.20/32 dev ${WG_IF_NAME}
...
function down() {
	...
	ip addr del 192.168.3.20/32 dev ${WG_IF_NAME}
...
# DOCS
#
# the target host should have following IPTables rules added (these work with firewall down)
# this will direct the vpn traffic out to the internet on the via target host (one having the eth0 interface, aka vpn server)
# if it is supposed to be 1 hope VPN (client -> vpn server -> internet) then this is all you need

# iptables -t nat -I POSTROUTING 1 -s 192.168.3.0/24 -o eth0 -j MASQUERADE
# iptables -I INPUT 1 -i wgvpn0 -j ACCEPT
# iptables -I FORWARD 1 -i eth0 -o wgvpn0 -j ACCEPT
# iptables -I FORWARD 1 -i wgvpn0 -o eth0 -j ACCEPT


# this is for 2 hops VPN (client -> vpn server 1 -> vpn server 2 -> internet)
